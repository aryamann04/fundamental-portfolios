...
from datapipeline import nasdaq_data, price
...
def rank_by(given_date, metric, **kwargs):
    df = nasdaq_data(given_date)
...
def year_backtest(metric, start_date):
    less_equal_zero, quintiles = rank_by(start_date, metric)
    ...
    for tickers in portfolio_tickers:
        ...
        for ticker in tickers:
            ticker_data = price(ticker, start_date, end_date)
...
def rebalanced_portfolio(metric, start_date='2000-06-30', end_date='2023-12-31'):
    ...
    while current_date < end_date:
        annual_returns, portfolios = year_backtest(metric, current_date.strftime('%Y-%m-%d'))
